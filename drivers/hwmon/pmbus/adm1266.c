...
static int adm1266_write_intel_hex(struct adm1266_data *data,
...
static int adm1266_verify_memory(struct adm1266_data *data)
{
	...
	cmd = 0;
	ret = pmbus_block_write(data->client, ADM1266_MEMORY_CRC, 1, &cmd);
	if (ret < 0)
		return ret;
...
static int adm1266_refresh_memory(struct adm1266_data *data)
{
	...
	cmd = 0x2;
	ret = pmbus_block_write(data->client, ADM1266_REFRESH_FLASH, 1, &cmd);
	if (ret < 0) {
...
static int adm1266_program_firmware(struct adm1266_data *data)
{
	...
	ret = adm1266_write_intel_hex(data, ADM1266_FIRMWARE_OFFSET,
...
static int adm1266_program_config(struct adm1266_data *data)
{
	...
	ret = adm1266_write_intel_hex(data, ADM1266_CONFIG_OFFSET,
				      ADM1266_CONFIG_SIZE);
	if (ret < 0) {
	...
	ret = pmbus_write_word_data(data->client, 0, ADM1266_GO_COMMAND, 0);
	if (ret < 0) {
				  ...
				  struct adm1266_data *master)
{
	...
	slave_ref = kzalloc(sizeof(*slave_ref), GFP_KERNEL);
	if (!slave_ref)
		return -ENOMEM;
...
static int adm1266_register(struct adm1266_data *data)
{
	...
	if (!fwnode_property_present(fw, "adi,master-adm1266")) {
		...
		master_ref = kzalloc(sizeof(*master_ref), GFP_KERNEL);
		if (!master_ref)
			return -ENOMEM;
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (!ret) {
		dev_err(&client->dev, "Chip firmware not written.");
